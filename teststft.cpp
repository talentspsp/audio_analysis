...
int main (int argc, char* argv[])
{
  ...
  //  ofstream datafd("testdata/stftin");
  ...
  size_t szr, szc;
  double* datain=bin2mat("testdata/80s_wavdata.bin", &szr, &szc);
  complex<double>* data=new complex<double>[szr*szc];
 for(i=0;i<szr*szc;i++)
    {
      data[i].re=datain[i];
      data[i].im=0;
  ...
  complex<double>* result=STFT(data, szr*szc, 1024, 1024,256,lenf,lent);
  cout<<"result:"<<endl;
  for(i=0;i<lent;i++)
    {
      for(j=0;j<lenf;j++)
